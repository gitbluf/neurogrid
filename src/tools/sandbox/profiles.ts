...
export function filterProjectEnvDenies(
  projectDir: string,
): { literal: string[]; regex: string[] } {
  const literals = ENV_DENY_PATTERNS.filter((pattern) => !pattern.includes("*"))
    .map((pattern) => path.join(projectDir, pattern))
  const regex = ENV_DENY_PATTERNS.filter((pattern) => pattern.includes("*"))
    .map((pattern) => {
...
function buildSandboxExecEnvDenyRules(
  ...
  const ruleLines = [...literal.map((item) => `(literal "${escapeSchemePath(item)}")`)]
